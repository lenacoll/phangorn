...
random.addition <- function(data, tree = NULL, method = "fitch") {
  ...
  for (i in remaining) {
    edge <- tree$edge
    #    f$traversetwice(edge, 0L) f$root_all_node(edge) score <- f$pscore_vec(edge,
    #    i)
...
indexNNI_fitch <- function(tree, offset = 2L * Ntip(tree), rooted = is.rooted(tree)) {
  ...
  #     a         d \       / e-----f       d is closest to root, f is root from
  #      subtree a,b,c /       \
  #     b         c     c(a,b,c,d,e,f)
  ...
  for (i in ind) {
    ...
    #    else if(rooted) cd <- c(cd, NA_integer_) else if(!rooted) ef <- c(i,
    #    cd[2]) else cd[2] <- f
...
optim.fitch <- function(tree, data, trace = 1, rearrangements = "NNI", log = NULL, ...) {
  ...
  if (nTips < 4L || star_tree) {
    nam <- names(data)
    if (star_tree) {
      tree <- stree(length(nam), tip.label = nam)
    } else {
      tree <- stree(nTips, tip.label = nam)
    }
  ...
  on.exit({
    ...
    if (!is.null(log)) {
      if (add_taxa) {
        duplicated_taxa <- attr(data, "duplicated")
        tree_list <- lapply(tree_list, function(tree) addTaxa(tree, duplicated_taxa))
        class(tree_list) <- "multiPhylo" # Restore class after lapply
      }
      write.tree(tree_list, file = log)
    }
    return(tree)
  })
  ...
  if (trace > 0) {
    cat(
      "Final p-score", pscore, "after ", swap,
      "nni operations \n"
    )
  }
}
